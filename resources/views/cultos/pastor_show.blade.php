@extends('layouts.app')
@section('titulo', 'Supervisión Pastoral - ' . $culto->nombre_culto)
...
{{-- SECCIÓN: MENSAJE PASTORAL --}}
<div style="background:#fff; border:2px solid #F5C518; border-radius:12px; overflow:hidden; margin-bottom:2rem;">
            ...
            <div style="display:flex; align-items:center; gap:0.75rem;">
                ...
                <div>
                    ...
                    <p style="font-size:13px; color:#555; margin:0;">
                        Orientación espiritual para los servidores de este culto
                    </p>
                </div>
            </div>
            @if($culto->mensaje)
    ...
    <div style="padding:1.5rem;">
        @if($culto->mensaje)
        <div style="background:#FFF8DC; border-left:4px solid #F5C518; padding:1.25rem; border-radius:8px; margin-bottom:1rem;">
            <p style="font-size:15px; color:#333; margin:0; line-height:1.7; font-style:italic;">
                "{{ $culto->mensaje }}"
            </p>
            @if($culto->mensajeAutor)
            <p style="font-size:12px; color:#777; margin:10px 0 0 0; text-align:right;">
        ...
        <div style="text-align:center; padding:1.5rem; background:#F8F9FA; border-radius:8px; margin-bottom:1rem;">
            <p style="color:#999; font-style:italic; margin:0; font-size:14px;">
                No hay mensaje pastoral para este culto aún
            </p>
        </div>
        @endif
        ...
        <form method="POST" action="{{ route('cultos.mensaje', $culto->id) }}">
            @csrf
            <div>
                <label style="font-size:14px; font-weight:600; color:#3a4255; display:block; margin-bottom:10px;">
                ...
                <textarea name="mensaje" rows="4"
                          ...
                          onblur="this.style.borderColor='#D1DCF0';">{{ old('mensaje', $culto->mensaje) }}</textarea>
            </div>
            <div style="margin-top:1rem; text-align:right;">
                <button type="submit" class="btn" style="background:#1A7A4A; color:#fff; padding:12px 24px; border:none; border-radius:8px; cursor:pointer; font-weight:500; font-size:14px;">
                    &#10003; {{ $culto->mensaje ? 'Actualizar Mensaje' : 'Guardar Mensaje Pastoral' }}
                </button>
            </div>
        </form>
    </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div style="display:flex; align-items:center; gap:0.75rem;">
                ...
                <div>
                    ...
                    <p style="font-size:13px; color:#555; margin:0;">
                        Supervisión de las asignaciones de servicio
                    </p>
                </div>
            </div>
...
<div style="margin-top:2rem; display:flex; gap:1rem; flex-wrap:wrap;">
    ...
    @if($usuario->tieneRol('secretario_general') || $usuario->tieneRol('pastor'))
    <form method="POST" action="{{ route('cultos.mensaje', $culto->id) }}" style="display:inline;">
        @csrf
        <input type="hidden" name="mensaje" value="">
        <button type="submit"
                onclick="return confirm('¿Eliminar el mensaje pastoral de este culto?')"
                style="background:#fff; color:#C0392B; padding:10px 20px; border:1px solid #C0392B; border-radius:8px; cursor:pointer; font-size:13px; font-weight:500;">
            &#128465; Eliminar Mensaje
        </button>
    </form>
    @endif
